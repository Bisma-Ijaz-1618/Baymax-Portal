...
function DeleteAccount({
  ...
  const deleteProfileMutation = useMutation(deleteProfile, {
    onSuccess: () => {
      queryClient.invalidateQueries(invalidQuery, { exact: true });
      setProfileData(null);
      setCallDelete(false);
    },
    onError: () => {
      setErrMsg(
        `There was an error while deleting ${invalidQuery} Profile. Please try again!`
      );
    },
  });
    ...
    >
      ...
      <Offcanvas.Body>
        ...
        <div className="mt-3 d-flex justify-content-center">
          ...
          <Button
            onClick={() =>
              deleteProfileMutation.mutate({ id: profileData._id })
            }
            variant="danger"
            disabled={disableButton}
          >
